...
angular.module('quizApp').controller('AdminSubjectsController', ['$location','$scope','$http','$routeParams', 'AdminSubjectsService', 'NavigationService', function($location, $scope, $http,$routeParams, AdminSubjectsService, NavigationService) {
    $scope.subjects = [];
    $scope.subject = {};
    ...
    $scope.navigateToManageTest  = function(subjectId){
        let currentPath = $location.path();
        let newPath;
      
        if (currentPath.includes('edit/')) {
            newPath = currentPath.replace(/edit\/\w+/, subjectId + '/test');
            console.log(newPath);
        } else {
            newPath = currentPath.endsWith('/') ? currentPath + subjectId : currentPath  + '/'+ subjectId +'/test';
        }
